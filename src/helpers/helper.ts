...
type IdTagStatusType = 'ACCEPTED' | 'BLOCKED' | 'EXPIRED' | 'INVALID' | 'CONCURRENT_TX';
type OCPPIdTagStatus = 'Accepted' | 'Blocked' | 'Expired' | 'Invalid' | 'ConcurrentTx';

export const idTagStatus = (status: string): OCPPIdTagStatus => {
  const map: Record<IdTagStatusType, OCPPIdTagStatus> = {
    ACCEPTED: "Accepted",
    BLOCKED: "Blocked",
    EXPIRED: "Expired",
    INVALID: "Invalid",
  ...
  return map[status as IdTagStatusType] ?? "Invalid";
}
